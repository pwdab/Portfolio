...
main (void)
{
  ...
  //printf("\tthreads/init.c..main(void) run_actions(argv)\n");
  run_actions (argv);
...
run_task (char **argv)
{
  //printf("\n\tthreads/init.c..run_task(char **argv)\n");
  const char *task = argv[1];
  
  printf ("Executing '%s':\n", task);
#ifdef USERPROG
  //printf("\n\tthreads/init.c..run_task(char **argv) userprog/process.c..process_execute(const char *file_name)\n");
  ...
  while (*argv != NULL)
    {
      ...
      //printf("\n\ta->name = %s\n", a->name);
      //printf("\tthreads/init.c..run_actions(char **argv) run_task(char **argv)\n");
      a->function (argv);
      argv += a->argc;
    }
